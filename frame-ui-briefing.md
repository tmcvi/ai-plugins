...
| F1c | ✅ **Applied 20 Sep 2026.** Phased hours, cost and revenue were silently dropped for any task whose `Adjust Responsible Role` (`ce745b81-…`) had been overridden to a role outside that task's `Role One…Five`: `Task Phase` (`15c055dd-…`) is gated by the same `Task Role Defined` Booleans, so it was blank on the cell the reassigned hours landed on and `[BY: -> 'Task Phase']` had nowhere to send them. Fixed by resolving the phase at **task** level — `[BY: -> 'Task Phase' [remove lastnonblank: 'Responsible Role']]` — in `Total Hours Estimate phased` (`391902e0-…`), `Phased Earnt Revenue` (`a517cd23-…`) and `Phased Cost` (`78ea54a9-…`). The six `Country Driver Data *` metrics share the idiom but sit upstream of the reassignment and were deliberately left alone. The `Phasing Leak Check` guard metric (`b0bc4d81-…`) is now live in `/14. Cashflow` and reads exactly 0.00 on all six populated versions; **outstanding:** surface it on `8. Cashflow Profile` with a conditional format on non-zero | Verified: all eight project versions now tie `Phased Earnt Revenue` to `Price (with Tasks)` and `Phased Cost` to `Cost (with Tasks)`; Project 7 recovers 2,304 h and 120,663.63 of revenue, Project 5 recovers 12 h. `Total Hours Estimate Study Phases` inherits the fix, so the FTE chain is conserved too | Applied |
