...
About 14 consultant-days for the four Frames plus the remaining model fixes and a day of SDK prototyping (step 0b is down from 2.5 days to 1, the blocking data work having been done), one developer with Pigment modelling and front-end JS, reviewed by Tom. Order is chosen so the demo-critical screens land first and the Cashflow Frame waits for its data fix.
...
| 0b | Model fixes from §11. **Applied already: F1b, F1c, most of F4** — the reconciliation exit criterion below is already met. Remaining: the `Phasing Leak Check` guard metric, F1 board scope, F3, the Change Log display property; F5 decided | 1 | ✅ `Phased Earnt Revenue` = `Price (with Tasks)` and `Phased Cost` = `Cost (with Tasks)` on all eight versions; Cashflow figures reconcile to Summary Budget for Project 4 (v1). Remaining: guard metric live on the Cashflow board, F3 twins built |
...
The Frames are designed against the model as extracted on 20 Sep. **F1b, F1c and most of F4 have since been applied directly in the live application and verified against live data** — all eight project versions now reconcile `Phased Earnt Revenue` to `Price (with Tasks)`. F1 has been **withdrawn** as a formula error and restated as a board-scoping fix. Nothing blocking remains: three fixes are required, one needs a decision and one is optional; the decisions below change the design and need an answer before step 1.
...
| F1c | ✅ **Applied 20 Sep 2026.** Phased hours, cost and revenue were silently dropped for any task whose `Adjust Responsible Role` (`ce745b81-…`) had been overridden to a role outside that task's `Role One…Five`: `Task Phase` (`15c055dd-…`) is gated by the same `Task Role Defined` Booleans, so it was blank on the cell the reassigned hours landed on and `[BY: -> 'Task Phase']` had nowhere to send them. Fixed by resolving the phase at **task** level — `[BY: -> 'Task Phase' [remove lastnonblank: 'Responsible Role']]` — in `Total Hours Estimate phased` (`391902e0-…`), `Phased Earnt Revenue` (`a517cd23-…`) and `Phased Cost` (`78ea54a9-…`). The six `Country Driver Data *` metrics share the idiom but sit upstream of the reassignment and were deliberately left alone. **Outstanding:** the `Phasing Leak Check` guard metric — MCP refused the create call, so add it in the UI (formula and spec in §9.2 of the architecture extract) | Verified: all eight project versions now tie `Phased Earnt Revenue` to `Price (with Tasks)` and `Phased Cost` to `Cost (with Tasks)`; Project 7 recovers 2,304 h and 120,663.63 of revenue, Project 5 recovers 12 h. `Total Hours Estimate Study Phases` inherits the fix, so the FTE chain is conserved too | Applied |
