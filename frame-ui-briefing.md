...
| 0b | Model fixes from §11 (F1, F1b, F3, F4; F5 decided) and revalidation of the FTE chain | 2.5 | Cashflow figures reconcile to Summary Budget for Project 4 (v1); Phased Earnt Revenue summed over all dimensions equals Price (with Tasks) |
...
The Frames are designed against the model as extracted on 20 Sep. Four fixes are blocking or required, one needs a decision and one is optional; the decisions below change the design and need an answer before step 1.

**Model fixes the Frames depend on**

| # | Fix | Why | Status |
| --- | --- | --- | --- |
| F1 | `Standard Milestone Payments` (`f2478498-…`) must sum to 100% across `Task Stage`; today it is eight 100%s (800%). `Milestone Amount` then multiplies whole-project revenue by each stage's 100%, giving 10.58m against a 554k budget | Every Cashflow figure and the Executive Summary chart are wrong until fixed — in both UIs | Blocking |
| F1b | In `Phased Earnt Revenue` (`a517cd23-…`), `Phased Cost` (`78ea54a9-…`) and `Total Hours Estimate Study Phases` (`21d0c1e4-…`), replace `IFDEFINED(Milestone_Period, 1)` with `Milestone_Period`. The flag double-books boundary months to two stages, so any total over `Task Stage` exceeds `Price (with Tasks)` — `Milestone Amount`, `Cash Receipts` in Monthly Billing mode and the FTE chain are all affected | Without this, F1 alone does not make step 0b's reconciliation true, and the main chart is wrong in both billing modes | Blocking |
...
| F4 | Set the `Change Log` display property to `Change ID` or a concatenation — `editItem` addresses rows by display name, so with GUIDs showing no Frame can edit a changelog row; move `5. Timeline Planning` out of `Not in Use` so the native and Frame versions sit side by side; remove the text-widget link to the `[OLD]` application | Load-bearing for Frame 1 writes, not cosmetic | Required |
