...
(function(){
  ...
  function render(){
    ...
    // de-collide the vertical end labels: nudge apart any nearer than one label width
    var LBLX={}; (function(){ var e=[]; for(var i=0;i<arr.length;i++){ var rr=arr[i]; e.push({m:rr.market,x:X(rr.reg!=null?rr.reg:rr.sale)}); } e.sort(function(a,b){return a.x-b.x;}); var gap=15, rb=W-14, lb=plotL-30; for(var i=1;i<e.length;i++){ if(e[i].x<e[i-1].x+gap)e[i].x=e[i-1].x+gap; } if(e.length&&e[e.length-1].x>rb){ e[e.length-1].x=rb; for(var j=e.length-2;j>=0;j--){ if(e[j].x>e[j+1].x-gap)e[j].x=e[j+1].x-gap; } } if(e.length&&e[0].x<lb){ e[0].x=lb; for(var k=1;k<e.length;k++){ if(e[k].x<e[k-1].x+gap)e[k].x=e[k-1].x+gap; } } for(var q2=0;q2<e.length;q2++)LBLX[e[q2].m]=e[q2].x; })();
    ...
    function drawLine(r,onTop){ var isAct=!!actMap[r.market]; if(onTop!==isAct)return; var col=isAct?C.red:C.charcoal; var op=isAct?1:(eng?0.16:0.55); var w=1.5+2.5*(r.alloc/maxA); var nds=nodesFor(r); var main=''; for(var i=0;i<6;i++){ var px=X(nds[i][0]),py=Y(nds[i][1]); main+=(i?' L':'M')+px.toFixed(1)+' '+py.toFixed(1); } var p1=el('path',{d:main,fill:'none',stroke:col,'stroke-width':w,'stroke-linejoin':'round','stroke-linecap':'round',opacity:op}); svg.appendChild(p1); var tx1=X(nds[5][0]),ty1=Y(5),tx2=X(nds[6][0]),ty2=Y(6); svg.appendChild(el('path',{d:'M'+tx1.toFixed(1)+' '+ty1.toFixed(1)+' L'+tx2.toFixed(1)+' '+ty2.toFixed(1),fill:'none',stroke:col,'stroke-width':w,'stroke-linecap':'round',opacity:op*0.5})); for(var n=0;n<7;n++){ var showTick=(laneHover===n); svg.appendChild(el('circle',{cx:X(nds[n][0]),cy:Y(n),r:(isAct?3.6:2.6),fill:col,opacity:op})); if(showTick&&(isAct||!eng)){ var dt=el('text',{x:X(nds[n][0]),y:Y(n)-7,fill:col,'font-family':SANS,'font-size':10,'font-weight':600,'text-anchor':'middle',opacity:Math.max(op,0.5)}); dt.textContent=fmtWeek(nds[n][0]); svg.appendChild(dt); } }
      ...
      var endX=X(nds[6][0]); var lblX=(LBLX[r.market]!=null?LBLX[r.market]:endX); var lop=isAct?1:(eng?0.3:0.72); svg.appendChild(el('path',{d:'M'+endX.toFixed(1)+' '+Y(6).toFixed(1)+' L'+endX.toFixed(1)+' '+(laneB-5).toFixed(1)+' L'+lblX.toFixed(1)+' '+(laneB+2).toFixed(1),fill:'none',stroke:col,'stroke-width':1,'stroke-linejoin':'round','stroke-linecap':'round',opacity:op*0.45})); var lyy=laneB+10; var elab=el('text',{x:lblX,y:lyy,fill:col,'font-family':SANS,'font-size':11,'font-weight':(isAct?700:600),'text-anchor':'start','dominant-baseline':'central',opacity:lop}); elab.setAttribute('transform','rotate(90 '+lblX.toFixed(1)+' '+lyy+')'); elab.textContent=r.market+' · '+Math.round(r.alloc).toLocaleString(); svg.appendChild(elab);
