...
(function () {
  ...
  function reshape(data){ var mk={}; if(!data||!data.labels)return mk; var cols=data.labels.columns||[], rows=data.labels.rows||[], cells=data.cells||[]; var ai=0, ci=1; for(var c=0;c<cols.length;c++){ var lb=lastLabel(cols[c]).toLowerCase(); if(lb.indexOf('alloc')>=0)ai=c; else if(lb.indexOf('cover')>=0)ci=c; } for(var r=0;r<rows.length;r++){ var lv=rows[r]; if(!Array.isArray(lv))lv=[lv]; if(lv.length<2)continue; var mdl=strOf(lv[0]); var market=strOf(lv[lv.length-1]); if(market==null)continue; var alloc=num(cells[ai]?cells[ai][r]:null); var cov=num(cells[ci]?cells[ci][r]:null); if(alloc==null||alloc<=0)continue; if(!mk[market])mk[market]={alloc:0,covW:0,covWsum:0,models:{}}; mk[market].alloc+=alloc; if(mdl!=null)mk[market].models[mdl]=(mk[market].models[mdl]||0)+alloc; if(cov!=null){ mk[market].covW+=cov*alloc; mk[market].covWsum+=alloc; } } var out={}; for(var m in mk){ var arr=[]; for(var mm in mk[m].models)arr.push({mdl:mm,units:mk[m].models[mm]}); arr.sort(function(a,b){return b.units-a.units;}); out[m]={alloc:mk[m].alloc, cov:(mk[m].covWsum>0?mk[m].covW/mk[m].covWsum:null), models:arr}; } return out; }
  ...
  function render(){
    ...
    for(var ni=0;ni<markets.length;ni++){ var mk=markets[ni], gc2=GEO[mk]; var x=X(gc2[0]), y=Y(gc2[1]); var cov2=D[mk].cov, col2=bandColor(cov2), r=rad(D[mk].alloc);
      ...
      (function(name,cv,cx,cy,rr,cc,pathEl){ function emph(ev){ nodeEls[name].setAttribute('r',rr+3); nodeEls[name].setAttribute('fill',rgba(cc,0.95)); if(pathEl){ pathEl.setAttribute('stroke',rgba(cc,0.7)); } var mods=D[name].models||[]; var mrows=''; for(var q=0;q<mods.length;q++){ mrows+=tipRow(mods[q].mdl, Math.round(mods[q].units).toLocaleString()); } tip.innerHTML='<div style="font:700 13px/1.3 '+SANS+';margin-bottom:4px;">'+esc(name)+'</div>'+tipRow('Allocated',Math.round(D[name].alloc).toLocaleString()+' cars')+tipRow('In-market cover',(cv!=null?(Math.round(cv*10)/10)+' wks · '+bandName(cv):'no demand'))+'<div style="margin-top:6px;margin-bottom:2px;color:'+C.grey60+';font-size:11px;font-weight:600;">Models sent</div>'+mrows; tip.style.opacity='1'; place(ev); } circ.addEventListener('mousemove',emph); circ.addEventListener('mouseenter',emph); circ.addEventListener('mouseleave',function(){ nodeEls[name].setAttribute('r',rr); nodeEls[name].setAttribute('fill',rgba(cc,0.82)); if(pathEl){ pathEl.setAttribute('stroke',rgba(cc,0.28)); } hideTip(); }); })(mk,cov2,x,y,r,col2,linkEls[mk]);
    }
  }
